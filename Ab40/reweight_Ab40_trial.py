...
save            = True
to_remove       = ['C', 'HA2', 'HA3']
reweighting_keys = ['CA', 'CB','JHNHA','HA','H','N','Rg']
blk_start = 2
blk_stop = 500
blk_step = 100
#blk_start = 10
...
kish_mini = 20
Kish_keys = ['CA', 'CB', 'JHNHA', 'HA', 'H', 'N', 'Rg']
Kish_ratio_vals = []
...
rg_i=np.dot(compare_dict_r['Rg'][0][1],initial_weights)
rg_r=np.dot(compare_dict_r['Rg'][0][1],final_weights)
rg_exp=expdata['Rg'][0][0]
rg_err=.14
print("Rg Exp:%6.2lf+_%6.2lf"%(rg_exp,rg_err),"Rg Initial:%6.2lf"%rg_i,"Rg Reweight:%6.2lf "%rg_r)

plt.hist(compare_dict_r['Rg'][0][1],bins=50,weights=final_weights,density='true',alpha=0.2,label='Rg Reweight',color='blue')
plt.hist(compare_dict_r['Rg'][0][1],bins=50,density='true',color='orange',histtype='step',label='Rg Initial')
